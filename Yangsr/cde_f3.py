...
class TextIndexer:
    def __init__(self, model_name="jxm/cde-small-v2"):
        # SentenceTransformer 대신 transformers 라이브러리를 직접 사용
    ...
    def build(self, embeddings_file="cde_minicorpus.pt"):
        try:
            ...
            if isinstance(embeddings_data, dict) and 'embeddings' in embeddings_data:
                ...
                # 텐서만 로드된 경우 chunks는 다시 생성해야 함 (필요에 따라)
            ...
            if not texts:
                self.idx = None
                return
            # 직접 구현한 _encode 함수 사용
            embs = self._encode(texts).numpy().astype("float32")
            dim = embs.shape[1]
            self.idx = faiss.IndexFlatIP(dim)
            self.idx.add(embs)
